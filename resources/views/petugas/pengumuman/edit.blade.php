        ...
        <div class="col-lg-10">
            ...
            <div class="card shadow-sm mb-4" style="border:none;border-radius:16px;background:linear-gradient(135deg,#8b5cf6,#7c3aed)">
                <div class="card-body text-center text-white py-4">
                    ...
                    <h3 class="font-weight-bold mb-2">Edit Pengumuman</h3>
                    <p class="mb-0" style="opacity:0.9">Perbarui informasi pengumuman</p>
                </div>
            </div>
            ...
            <div class="card shadow-sm" style="border:none;border-radius:16px">
                <div class="card-body p-4">
                    <form action="{{ route('petugas.pengumuman.update', $pengumuman->id) }}" method="POST" id="formPengumuman">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="judul" class="form-control form-control-lg @error('judul') is-invalid @enderror"
                                   value="{{ old('judul', $pengumuman->judul) }}" placeholder="Contoh: Rapat Anggota Tahunan 2026" required
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea name="isi" rows="8" class="form-control @error('isi') is-invalid @enderror"
                                      ...
                                      style="border-radius:12px;border:2px solid #e5e7eb;padding:14px 18px;line-height:1.8">{{ old('isi', $pengumuman->isi) }}</textarea>
                            @error("isi")
                            <div class="invalid-feedback d-block">
                                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="hari" class="form-control @error('hari') is-invalid @enderror" required
                                    ...
                                    <option value="Senin" {{ old('hari', $pengumuman->hari)=='Senin'?'selected':'' }}>Senin</option>
                                    <option value="Selasa" {{ old('hari', $pengumuman->hari)=='Selasa'?'selected':'' }}>Selasa</option>
                                    <option value="Rabu" {{ old('hari', $pengumuman->hari)=='Rabu'?'selected':'' }}>Rabu</option>
                                    <option value="Kamis" {{ old('hari', $pengumuman->hari)=='Kamis'?'selected':'' }}>Kamis</option>
                                    <option value="Jumat" {{ old('hari', $pengumuman->hari)=='Jumat'?'selected':'' }}>Jumat</option>
                                    <option value="Sabtu" {{ old('hari', $pengumuman->hari)=='Sabtu'?'selected':'' }}>Sabtu</option>
                                    <option value="Minggu" {{ old('hari', $pengumuman->hari)=='Minggu'?'selected':'' }}>Minggu</option>
                                </select>
                                @error("hari")
                                <div class="invalid-feedback d-block">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="time" name="jam" class="form-control @error('jam') is-invalid @enderror"
                                       value="{{ old('jam', $pengumuman->jam) }}" required
                                       style="border-radius:12px;border:2px solid #e5e7eb;padding:12px 16px">
                                @error("jam")
                                <div class="invalid-feedback d-block">
                                    <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="number" name="tahun" class="form-control @error('tahun') is-invalid @enderror"
                                   value="{{ old('tahun', $pengumuman->tahun) }}" min="2020" max="2100" required
                                   style="border-radius:12px;border:2px solid #e5e7eb;padding:12px 16px">
                            @error("tahun")
                            <div class="invalid-feedback d-block">
                                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                        ...
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="pembuat" class="form-control @error('pembuat') is-invalid @enderror"
                                   value="{{ old('pembuat', $pengumuman->pembuat) }}" placeholder="Nama lengkap pembuat surat" required
                                   style="border-radius:12px;border:2px solid #e5e7eb;padding:12px 16px">
                            @error("pembuat")
                            <div class="invalid-feedback d-block">
                                <i class="fas fa-exclamation-circle mr-1"></i>{{ $message }}
                        ...
                        <div class="form-group mb-4">
                            ...
                            <select name="jenis" class="form-control" style="border-radius:12px;border:2px solid #e5e7eb;padding:12px 16px">
                                <option value="info" {{ old('jenis', $pengumuman->jenis)=='info'?'selected':'' }}>Info</option>
                                <option value="warning" {{ old('jenis', $pengumuman->jenis)=='warning'?'selected':'' }}>Peringatan</option>
                                <option value="success" {{ old('jenis', $pengumuman->jenis)=='success'?'selected':'' }}>Sukses</option>
                                <option value="danger" {{ old('jenis', $pengumuman->jenis)=='danger'?'selected':'' }}>Penting</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group mb-4">
                            <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input" id="is_aktif" name="is_aktif" {{ old('is_aktif', $pengumuman->is_aktif) ? 'checked' : '' }}>
