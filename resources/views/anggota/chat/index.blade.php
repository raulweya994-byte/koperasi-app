            ...
            <div class="card shadow-sm" style="border-radius:16px;border:none;background:linear-gradient(135deg,#10b981,#059669)">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center text-white">
                        ...
                        <div>
                            <h3 class="mb-1 font-weight-bold">Chat dengan Admin</h3>
                            <p class="mb-0" style="opacity:0.9">Hubungi administrator untuk bantuan</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-0">
                        ...
                        <div class="chat-list">
                            ...
                            <div class="chat-item" data-user-id="{{ $user->id }}" data-user-name="{{ $user->name }}" data-user-role="{{ $user->role }}">
                                <div class="chat-avatar">
                                    ...
                                    @else
                                        <div class="avatar-placeholder" style="background: {{ $user->role == 'admin' ? 'linear-gradient(135deg, #dc3545, #c82333)' : ($user->role == 'pimpinan' ? 'linear-gradient(135deg, #8b5cf6, #7c3aed)' : 'linear-gradient(135deg, #06b6d4, #0891b2)') }}">
                                ...
                                <div class="chat-info">
                                    <div class="chat-header">
                                        <div>
                                            ...
                                            <span class="badge badge-{{ $user->role == 'admin' ? 'danger' : ($user->role == 'pimpinan' ? 'purple' : 'info') }}" style="font-size: 10px; padding: 3px 8px;">
                                                {{ $user->role == 'admin' ? 'ADMIN' : ($user->role == 'pimpinan' ? 'PIMPINAN' : 'PETUGAS') }}
                                            </span>
                                        </div>
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <h5>Tidak Ada Staff</h5>
                            <p>Belum ada admin, pimpinan, atau petugas yang terdaftar</p>
                        </div>
                    @endif
                </div>
            </div>
...
function loadMessages(userId) {
    $.ajax({
        url: `/anggota-portal/chat/${userId}`,
        method: 'GET',
...
$(document).on('submit', '#chatForm', function(e) {
    ...
    const url = editingMessageId ? `/anggota-portal/chat/${editingMessageId}` : '/anggota-portal/chat';
...
setInterval(function() {
    if (currentUserId && $('#chatModal').hasClass('show') && !isSubmitting && !editingMessageId) {
        ...
        $.ajax({
            url: `/anggota-portal/chat/${currentUserId}`,
            method: 'GET',
            success: function(response) {
                if (response.success) {
