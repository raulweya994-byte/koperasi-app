...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card shadow-sm" style="border-radius:16px;border:none;background:linear-gradient(135deg,#1a3a6e,#2d5aa0)">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center text-white">
                        ...
                        <div>
                            <h3 class="mb-1 font-weight-bold">Chat & Pesan</h3>
                            <p class="mb-0" style="opacity:0.9">Komunikasi dengan Admin, Petugas, Koperasi, dan Anggota</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card shadow-sm" style="border-radius:16px;border:none">
                <div class="card-header" style="background:linear-gradient(135deg,#f8f9fa,#e9ecef);border-radius:16px 16px 0 0;border:none;padding:20px">
                    <h5 class="mb-0 font-weight-bold" style="color:#1a3a6e">
                        <i class="fas fa-list mr-2"></i>Daftar Percakapan
                    </h5>
                </div>
                <div class="card-body p-0">
                        ...
                        <div class="chat-list">
                            ...
                            <div class="chat-item" data-user-id="{{ $user->id }}" data-user-name="{{ $user->name }}" data-user-role="{{ $user->role }}">
                                <div class="chat-avatar">
                                    ...
                                    @elseif($user->anggota && $user->anggota->foto)
                                        <img src="{{ asset('storage/'.$user->anggota->foto) }}" alt="{{ $user->name }}">
                                    @else
                                        <div class="avatar-placeholder" style="background: {{ $user->role == 'admin' ? 'linear-gradient(135deg, #dc3545, #c82333)' : ($user->role == 'petugas' ? 'linear-gradient(135deg, #06b6d4, #0891b2)' : ($user->role == 'koperasi' ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'linear-gradient(135deg, #10b981, #059669)')) }}">
                                ...
                                <div class="chat-info">
                                    <div class="chat-header">
                                        <div>
                                            ...
                                            <span class="badge badge-{{ $user->role == 'admin' ? 'danger' : ($user->role == 'petugas' ? 'info' : ($user->role == 'koperasi' ? 'warning' : 'success')) }}" style="font-size: 10px; padding: 3px 8px;">
                                                {{ ucfirst($user->role) }}
                                            </span>
                                        </div>
                    ...
                    @else
                        <div class="empty-state">
                            ...
                            <p>Pilih kontak di bawah untuk memulai chat</p>
                        </div>
                    @endif
                </div>
            </div>
...
<div class="modal fade" id="chatModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content" style="border-radius:16px;border:none;height:600px;display:flex;flex-direction:column">
            <div class="modal-header" style="background:linear-gradient(135deg,#1a3a6e,#2d5aa0);color:white;border-radius:16px 16px 0 0;padding:20px">
            ...
            <div class="modal-body p-0" style="flex:1;overflow:hidden;display:flex;flex-direction:column">
                ...
                <div style="padding:20px;border-top:1px solid #e5e7eb;background:white">
                    ...
                    <form id="chatForm" class="d-flex gap-2">
                        ...
                        <button type="submit" id="sendBtn" class="btn btn-primary" style="border-radius:25px;padding:12px 30px;white-space:nowrap">
...
function loadMessages(userId) {
    $.ajax({
        url: `/pimpinan/chat/messages/${userId}`,
        method: 'GET',
...
function renderChatModal(user, messages) {
    ...
    const roleColors = {
        'admin': { bg: 'linear-gradient(135deg, #dc3545, #c82333)', badge: 'ADMIN' },
        'petugas': { bg: 'linear-gradient(135deg, #06b6d4, #0891b2)', badge: 'PETUGAS' },
        'koperasi': { bg: 'linear-gradient(135deg, #f59e0b, #d97706)', badge: 'KOPERASI' },
        'anggota': { bg: 'linear-gradient(135deg, #10b981, #059669)', badge: 'ANGGOTA' }
    };
    const roleColor = roleColors[user.role] || roleColors['anggota'];
    
    $('#chatUserAvatar').text(initial).css('background', roleColor.bg);
    $('#chatUserName').text(user.name);
    $('#chatUserRole').html(`<span class="badge badge-light">${roleColor.badge}</span>`);
...
$(document).on('submit', '#chatForm', function(e) {
    ...
    const url = editingMessageId ? `/pimpinan/chat/update/${editingMessageId}` : '/pimpinan/chat/send';
...
function deleteMessage(messageId) {
    ...
    $.ajax({
        url: `/pimpinan/chat/delete/${messageId}`,
        method: 'DELETE',
        data: {
            _token: '{{ csrf_token() }}'
        },
...
setInterval(function() {
    if (currentUserId && $('#chatModal').hasClass('show') && !isSubmitting && !editingMessageId) {
        ...
        $.ajax({
            url: `/pimpinan/chat/messages/${currentUserId}`,
            method: 'GET',
            success: function(response) {
                if (response.success) {
