                ...
                <div class="card-body p-4">
                    <div class="d-flex align-items-center text-white">
                        ...
                        <div>
                            ...
                            <p class="mb-0" style="opacity:0.9">Komunikasi dengan Pimpinan, Petugas, Koperasi, dan Anggota</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card shadow-sm" style="border-radius:16px;border:none">
                <div class="card-header" style="background:linear-gradient(135deg,#f8f9fa,#e9ecef);border-radius:16px 16px 0 0;border:none;padding:20px">
                    <h5 class="mb-0 font-weight-bold" style="color:#10b981">
                        ...
                        <div class="chat-list">
                            ...
                            <div class="chat-item" data-user-id="{{ $user->id }}" data-user-name="{{ $user->name }}" data-user-role="{{ $user->role }}">
                                <div class="chat-avatar">
                                    ...
                                    @else
                                        <div class="avatar-placeholder" style="background: {{ $user->role == 'pimpinan' ? 'linear-gradient(135deg, #8b5cf6, #7c3aed)' : ($user->role == 'petugas' ? 'linear-gradient(135deg, #06b6d4, #0891b2)' : ($user->role == 'koperasi' ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'linear-gradient(135deg, #10b981, #059669)')) }}">
                                ...
                                <div class="chat-info">
                                    <div class="chat-header">
                                        <div>
                                            ...
                                            <span class="badge badge-{{ $user->role == 'pimpinan' ? 'purple' : ($user->role == 'petugas' ? 'info' : ($user->role == 'koperasi' ? 'warning' : 'success')) }}" style="font-size: 10px; padding: 3px 8px;">
                                                {{ $user->role == 'pimpinan' ? 'PIMPINAN' : ($user->role == 'petugas' ? 'PETUGAS' : ($user->role == 'koperasi' ? 'KOPERASI' : 'ANGGOTA')) }}
                                            </span>
                                        </div>
...
<div class="modal fade" id="chatModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content" style="border-radius:16px;border:none;height:600px;display:flex;flex-direction:column">
            <div class="modal-header" style="background:linear-gradient(135deg,#10b981,#059669);color:white;border-radius:16px 16px 0 0;padding:20px">
            ...
            <div class="modal-body p-0" style="flex:1;overflow:hidden;display:flex;flex-direction:column">
                ...
                <div style="padding:20px;border-top:1px solid #e5e7eb;background:white">
                    ...
                    <form id="chatForm" class="d-flex gap-2">
                        ...
                        <button type="submit" id="sendBtn" class="btn btn-success" style="border-radius:25px;padding:12px 30px;white-space:nowrap">
...
function loadMessages(userId) {
    $.ajax({
        url: `/admin/chat-messages/${userId}`,
        method: 'GET',
        success: function(response) {
            if (response.success) {
                renderChatModal(response.data);
            }
        },
        error: function() {
...
function renderChatModal(messages) {
    ...
    const roleColors = {
        'pimpinan': { bg: 'linear-gradient(135deg, #8b5cf6, #7c3aed)', badge: 'PIMPINAN' },
    ...
    const roleColor = roleColors[currentUserRole] || roleColors['anggota'];
...
$(document).on('submit', '#chatForm', function(e) {
    ...
    const url = editingMessageId ? `/admin/chat/${editingMessageId}` : '/admin/chat';
